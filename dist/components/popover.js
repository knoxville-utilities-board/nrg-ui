...
import { offset, arrow, size, computePosition } from '@floating-ui/dom';
...
class Popover extends Component {
  ...
  #_isShown = (i(this, "_isShown"), undefined);
  get hasArrow() {
  ...
  get middleware() {
    ...
    if (this.args.fullWidth) {
      const expandWidth1 = size({
        apply({
          rects: rects1,
          elements: elements1
        }) {
          Object.assign(elements1.floating.style, {
            minWidth: `${rects1.reference.width}px`
          });
        }
      });
      middleware1.push(expandWidth1);
    }
    return middleware1;
  }
  get side() {
  ...
  show = async evtOrInput1 => {
    if (this.isShown) {
      return;
    }
    this._isShown = true;
    await this.args.onShow?.();
    if (evtOrInput1 instanceof HTMLInputElement) {
      this._control = evtOrInput1;
      this.showPopover();
    } else if (evtOrInput1 instanceof Event && evtOrInput1.currentTarget instanceof HTMLElement) {
      this._control = evtOrInput1.currentTarget;
      this.showPopover();
    }
  };
  hide = async () => {
