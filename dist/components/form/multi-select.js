...
class MultiSelect extends BoundValue {
  ...
  static {
    setComponentTemplate(precompileTemplate("\n    <div class=\"card multi-select border-0\">\n      <Select @binding={{bind this.self \"lastSelection\"}} @closeOnSelect={{this.closeOnSelect}} @defaultTextKey=\"nrg.multi-select.defaultText\" @disabled={{@disabled}} @id={{@id}} @isInvalid={{@isInvalid}} @isWarning={{@isWarning}} @loading={{@loading}} @noOptionsText={{@noOptionsText}} @noOptionsTextKey={{@noOptionsTextKey}} @options={{this.availableOptions}} @scrollable={{@scrollable}} @onChange={{this.addItem}} ...attributes @serializationPath={{null}}>\n        <:empty>\n          {{#if this.value.length}}\n            {{#if (has-block \"display\")}}\n              {{yield this.value to=\"display\"}}\n            {{else}}\n              {{#each this.selectedOptions as |option i|}}\n                {{#if (has-block \"selection\")}}\n                  {{yield (hash value=option.raw Remove=(component RemoveButton disabled=@disabled onClick=(fn this.removeItem option i))) to=\"selection\"}}\n                {{else}}\n                  <span class=\"badge text-bg-secondary d-inline-flex align-items-center\">\n                    {{option.label}}\n                    <RemoveButton @disabled={{@disabled}} @onClick={{fn this.removeItem option i}} />\n                  </span>\n                {{/if}}\n              {{/each}}\n            {{/if}}\n          {{else}}\n            {{#if (has-block \"empty\")}}\n              {{yield to=\"empty\"}}\n            {{else}}\n              {{this.defaultText}}\n            {{/if}}\n          {{/if}}\n        </:empty>\n        <:option as |option|>\n          {{#if (has-block \"option\")}}\n            {{yield option.raw to=\"option\"}}\n          {{else}}\n            {{option.label}}\n          {{/if}}\n        </:option>\n        <:menu as |Menu|>\n          {{#if (has-block \"menu\")}}\n            {{yield Menu to=\"menu\"}}\n          {{/if}}\n        </:menu>\n      </Select>\n    </div>\n  ", {
      strictMode: true,
      scope: () => ({
        Select,
        bind,
