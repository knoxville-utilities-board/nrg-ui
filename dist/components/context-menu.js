...
class ContextMenu extends Component {
  ...
  static {
    setComponentTemplate(precompileTemplate("\n    <Dropdown class=\"context-menu\" @alignment={{@alignment}} @disabled={{@disabled}} @closeOnSelect={{@closeOnSelect}} @controlElement={{@controlElement}} @flip={{@flip}} @fullWidth={{@fullWidth}} @hasIcon={{@hasIcon}} @loading={{@loading}} @offset={{@offset}} @scrollable={{@scrollable}} @side={{@side}} @onHide={{@onHide}} @onShow={{@onShow}} ...attributes>\n      <:control as |visibility|>\n        <i aria-disabled={{if @disabled \"true\"}} class=\"bi bi-three-dots-vertical p-1\" disabled={{@disabled}} role=\"button\" title={{t \"nrg.navbar.toggleContextMenu\"}} {{on \"click\" visibility.toggle}} />\n      </:control>\n      <:menu as |Menu|>\n        <div class=\"dropdown-top\" {{!-- @glint-expect-error Modifier types are currently not correct --}} {{onInsert this.register Menu}} {{onDestroy this.unregister}}>\n          {{yield Menu}}\n        </div>\n        <div class=\"dropdown-bottom\"></div>\n      </:menu>\n    </Dropdown>\n  ", {
      strictMode: true,
      scope: () => ({
        Dropdown,
        t,
