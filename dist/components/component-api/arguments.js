...
const TextAreaArgument = setComponentTemplate(precompileTemplate("<BaseArgument @alias={{@alias}} @defaultValue={{@defaultValue}} @description={{@description}} @displayType={{@displayType}} @model={{@model}} @name={{@name}} @options={{@options}} @type={{or @type \"String\"}} @typeLink={{@typeLink}} @value={{@value}} @onInput={{@onInput}} as |base|>\n  {{#unless @hideControl}}\n    <TextArea @binding={{bind base \"value\"}} />\n  {{/unless}}\n\n  {{yield base base.value}}\n</BaseArgument>", {
  strictMode: true,
  scope: () => ({
    BaseArgument,
    or,
