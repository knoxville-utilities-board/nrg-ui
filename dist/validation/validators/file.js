...
class FileValidator extends BaseValidator {
  ...
  fileIsAllowed(value, options) {
    ...
    return options.some(option => option.toLowerCase() === fileExtension || this.checkMimeType(option, value.type));
  }
  checkFileIsAccepted(value, options) {
    ...
    if (!typeIsAllowed) {
      ...
      return {
        key: 'nrg.validation.file.acceptedTypes',
        types
      };
    }
    return true;
  ...
  checkFileIsUnaccepted(value, options) {
    ...
    if (typeIsNotAllowed) {
      ...
      return {
        key: 'nrg.validation.file.unacceptedTypes',
        types
      };
    }
    return true;
