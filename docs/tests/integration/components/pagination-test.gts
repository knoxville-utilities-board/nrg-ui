...
module('Integration | Component | pagination', function (this: TestContext, hooks) {
  ...
  test('it works', async function (this: TestContext, assert) {
    ...
    await render(<template><Pagination @meta={{meta}} @onChangePage={{setPage}} /></template>);

    assert.dom('[data-test-previous]').hasClass('disabled', 'previous is disabled');
    assert
    ...
    assert.dom('[data-test-next]').doesNotHaveClass('disabled', 'next is enabled');

    await click('[data-test-next]');

    assert.dom('[data-test-previous]').doesNotHaveClass('disabled', 'previous is enabled');
    assert
      .dom('[data-test-previous] + .page-item > .page-link')
      .hasClass('disabled', 'first slot is disabled')
      .matchesText(/\s/, 'first slot is empty');
    ...
    assert.verifySteps(
      ['start: 25', 'start: 50', 'start: 75', 'start: 100', 'start: 75', 'start: 25'],
  ...
  test('page sizes are supported', async function (this: TestContext, assert) {
    ...
    assert.dom('.selected-display').containsText('50 per page', 'selected option is 50');
    ...
    assert.dom('.selected-display').containsText('200 per page', 'selected option is 200');
  ...
  test('jump to page is supported', async function (this: TestContext, assert) {
    ...
    await render(
      <template>
        <Pagination @meta={{meta}} @enablePageJump={{true}} @onChangePage={{setPage}} />
      </template>,
    );
    ...
    assert.dom('[data-test-jump-to]').matchesText(/Page\s+of 4/, 'total pages');
  });
});
