...
module('Integration | Component | icon', function (hooks) {
  ...
  test('icon renders without circular background', async function () {
    await render(<template><Icon class="test" @type="bi-check" @color="primary" /></template>);
    ...
    assert.dom('div div i').hasClass('bi-check', 'Type param renders icon correctly');
    assert.dom('div div i').hasClass('text-primary', 'Color param renders text color correctly');
  });

  test('icon renders with circular background', async function () {
    await render(
      <template>
        <Icon class="test" @type="bi-check" @color="warning" @circular={{true}} />
      </template>,
    );
    assert
      .dom('.test div')
      .hasClass('rounded-circle', 'Icon has rounded-circle class when @circular is true');
    assert
      .dom('.test div')
  ...
  test('icon renders at a different size background', async function () {
    await render(
      <template><Icon class="test" @type="bi-check" @color="primary" @size="3" /></template>,
    );
