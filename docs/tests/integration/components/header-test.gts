...
module('Integration | Component | header', function (hooks) {
  ...
  test('it renders content within the correct named blocks', async function () {
    ...
    assert
      ...
      .hasText('Left side content', 'Content is rendered in correct block with correct text');
    assert
      .dom('.col.d-flex.app-bar-center p')
      .hasText('center content', 'Content is rendered in correct block with correct text');
    assert
      .dom('.col.d-flex.app-bar-right p')
      .hasText('right side content', 'Content is rendered in correct block with correct text');
    assert
  ...
  test('it renders content in the correct order', async function () {
    ...
    assert.dom('div div:nth-of-type(4) p').hasText('mobile drop section content');
  });
});
